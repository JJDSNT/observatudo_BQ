...
| Cube.js como `apps/api`, scaffold real (não placeholder), acesso só a `gold` | Evita reimplementar manualmente SQL/agregações em cada rota Next.js; dá um modelo semântico (medidas/dimensões) reutilizável. Como Cube.js (JS) e o DW (Python) só se relacionam via contrato de dados (tabelas do dataset `gold`), o schema dos cubos não dependia da decisão de deploy — por isso já existiu como app real antes de o deploy (self-hosted via Cloud Run) ser decidido e aplicado. |
